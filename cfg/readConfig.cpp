...
double sigma;                   //!< Parameters for the quasi-geostrophic model
gsl_vector *ci;                 //!< Parameters Ci for the quasi-geostrophic model
gsl_vector *li;                 //!< Dampings for the quasi-geostrophic model
...
// Continuation
double eps;                     //!< Tracking tolerance
int maxIter;                    //!< Maximum number of iterations for correction
int numShoot;                   //!< Number of shoots
double sigmaStep;               //!< Step size of parameter for continuation
double sigmaMin;                //!< Lower limit to which to continue
double sigmaMax;                //!< Upper limit to which to continue
...
readConfig(const char *cfgFileName)
{
  ...
  try {
    ...
    const Setting &ciSetting = cfg.lookup("model.ci");
    ci = gsl_vector_alloc(7);
    std::cout << "ci = [";
    for (size_t i =0; i < (size_t) 7; i++)
      {
	gsl_vector_set(ci, i, ciSetting[i]);
	std::cout << gsl_vector_get(ci, i) << " ";
      }
    std::cout << "]" << std::endl;
    const Setting &liSetting = cfg.lookup("model.li");
    li = gsl_vector_alloc(4);
    std::cout << "li = [";
    for (size_t i =0; i < (size_t) 4; i++)
      {
	gsl_vector_set(li, i, liSetting[i]);
	std::cout << gsl_vector_get(li, i) << " ";
      }
    std::cout << "]" << std::endl;
    
    strcpy(cpyBuffer, caseName);
    sprintf(caseName, "%s", cpyBuffer);
	...
	eps = cfg.lookup("continuation.eps");
	...
	sigmaStep = cfg.lookup("continuation.sigmaStep");
	std::cout << "sigmaStep = " << sigmaStep << std::endl;
	sigmaMin = cfg.lookup("continuation.sigmaMin");
	std::cout << "sigmaMin = " << sigmaMin << std::endl;
	sigmaMax = cfg.lookup("continuation.sigmaMax");
      ...
      else
	{
	  ...
	  sprintf(gridPostfix, "%s_n%dminmax", cpyBuffer,
		  gsl_vector_uint_get(nx, d));
	  strcpy(cpyBuffer, boxPostfix);
	  sprintf(gridPostfix, "%s_minmax", cpyBuffer);
	}
    }
    strcpy(cpyBuffer, gridPostfix);    
    sprintf(gridPostfix, "%s%s%s", srcPostfix, obsName, cpyBuffer);
    strcpy(cpyBuffer, boxPostfix);    
    sprintf(boxPostfix, "%s%s%s", srcPostfix, obsName, cpyBuffer);
